...
static       Vector3 Light(-1.40,0.0,3.75);   //Point Light-Source Position
...
rayObject(CObj *ob, const Vector3 &r, const Vector3 &o){
  ...
  } else if (tp == TYPE_SQUARE) {
      return ob->calcSquareIntersection(r, o);
  }
...
surfaceNormal(CObj *ob, const Vector3 &P, const Vector3 &Inside){
  ...
  } else if (ob->getType() == TYPE_PLANE || ob->getType() == TYPE_SQUARE) {
...
calcPixelColor(float x, float y){
  ...
  if (istat.dist >= NOT_INTERSECTED){ /* modified */
           return rgb;
  }
  ...
  if (lightPhotons){
    ...
    if (lht_stat.obj == org_stat.obj) {
	  ...
	  Vector3 ref = reflect(lht_stat.obj, pnt, pnt - Light, Light);
	  //intensity += pow(dot(ref, ray), 10);
    }
...
gatherPhotons(const Vector3 &p, CObj *ob)
{
  ...
  static const float sqRadius = 0.55; 
  float k = 1	;
  ...
  for (int i = 0; i < numPhotons[id]; i++) {
    ...
    if (cur_dist < sqRadius) {  
	  float cosine = -dot(N, photons[id][i][1]);
      //float weight = max(0.0, -dot(N, photons[id][i][1]) );
	  float weight = max(0.0, 1 - cur_dist / sqRadius / k);
      weight *= cosine / ((1 - 2 / 3 / k) * PI * sqRadius * sqRadius);
	  
	  Vector3 ref = photons[id][i][1] - N * (2 * dot(photons[id][i][1], N));
      ref.normalize();
	  float intensity = pow(dot(ref, photons[id][i][1]), 5);
...
void emitPhotons(){
  ...
  const int num_photon = view3D ? nrPhotons * 3.0 : nrPhotons;
    for (double q = Light.z() - 0.2; q < Light.z() + 0.2; q = q + 0.05) {
        for (double w = Light.y() - 0.2; w < Light.y() + 0.2; w = w + 0.05) {
            Vector3 templ(Light.x(), w, q);
  for (int i = 0; i < num_photon; i++){
    ...
    ray = randDir(1.0);
      if(squareObj) {
          while (ray.x() / fabs(ray.y()) >= -(0.05 / 0.2) || ray.x() / fabs(ray.z()) >= -(0.05 / 0.2)) {
              ray = randDir(1.0);
          }
      } else {
          if (!singleLight) {
      ...
      Vector3 from = (singleLight || squareObj)? Light : templ;
   ...
   /* while (from.y() >= Light.y()) {
    ...
    while (istat.dist < NOT_INTERSECTED && bounces <= nrBounces){
      ...
      int ref = 0;
      while (istat.obj->getOptics() != OPT_NONE && ref < reflection_limit){
        if(istat.obj->getOptics() == OPT_REFLECT) 
         {
             ray = reflect(istat.obj, pnt, ray, from);
         }
        else               
         {
             ray = refract(istat.obj, pnt, ray, from, refractive);
         }
        ref++;
      ...
      drawPhoton(rgb, pnt);
        if (istat.obj->getType() != TYPE_SQUARE) { /* modified */
...
void drawPhoton(const Vector3 &rgb, const Vector3 &p){           //Photon Visualization
  if (view3D && p[2] > 1.0){                       //Only Draw if In Front of Camera
...
onKeyPress(unsigned char key,int, int) {
  switch(key) {
    case 49 /*1 ray-tracing */ : view3D = false; lightPhotons = false; break;
    case 50 /*2 without square light object*/ : squareObj = false;singleLight = false; view3D = false; lightPhotons = true; Light = Vector3(-1.40,0.0,3.75); nrObjects = nrObjects0; break;
    case 51 /*3 with square light object*/ : squareObj = true;singleLight = false; view3D = false; lightPhotons = true; Light = Vector3(-1.40,0.0,3.75); nrObjects = nrObjects0 + 1; break;
    case 52 /*4 diffuse point light*/ : squareObj = false;singleLight = true; view3D = false; lightPhotons = true; Light = Vector3(0.0, 0.0, 3.75); nrObjects = nrObjects0; break;
      
    case 53 /*5 photon map*/ : view3D = true; break;
      default     : return;
  }
  resetRender();
...
void initObje() {
  ...
  for(int i=0; i<5; i++) {
    ...
    objects.push_back(ob);
        ...
        ob = new CObj(TYPE_SQUARE, nrObjects++, v_square[0]); //square Light source object
        objects.push_back(ob);
    nrObjects0 = nrObjects - 1;
  //--  set optical properties
  objects[0]->setOptics(OPT_REFLECT);
  objects[1]->setOptics(OPT_REFLECT);
    
  objects[2]->setOptics(OPT_REFRACT);
  objects[2]->setRefractive(3.0f);
