...
CObj::CObj(int tp, int idx, float *cod) {
  ...
  case TYPE_PLANE  : ncods=2; break;
      case TYPE_SQUARE : ncods=6; break;
  }
...
double
CObj::calcSquareIntersection(const Vector3 &r, const Vector3 &o) {
    int axis = (int) coords[0];
    if(r[axis] != 0.0){
        double temp = (coords[1] - o[axis]) / r[axis];
        if ((temp * r[1] + o[1]) >= coords[2] && (temp * r[1] + o[1]) <= coords[3] && (temp * r[2] + o[2]) >= coords[4] && (temp*r[2] + o[2]) <= coords[5]) {
            return temp;
        }
    }
    return NOT_INTERSECTED;
